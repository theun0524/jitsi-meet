...
function _updateReceiverVideoConstraints({ getState }) {
    ...
    // only forward participants on the viewport by setting custom constraints, and set default constraint
    // for others to 0
    const parsInPage = map(filter(getCurrentPage(state), p => !p.local), 'id');
    const constraintHeight = (selectedEndpoints?.length === 1)? VIDEO_QUALITY_LEVELS.LOW : maxFrameHeight;
    parsInPage.forEach(id => receiverConstraints.constraints[id] = { "maxHeight": constraintHeight });
