...
const VideoLayout = {
    ...
    onVideoMute(id) {
        if (APP.conference.isLocalId(id)) {
            ...
            this.moveMutedRemoteVideoToTheEndofDOM();
        }
     ...
     */
    moveMutedRemoteVideoToTheEndofDOM() {
        ...
        const allVideosContainers = document.getElementById('filmstripRemoteVideosContainer');
        
        // append the muted video to the end of DOM
        const mutedParticipants = getVideoMutedParticipants(APP.store.getState())
        const mutedDOMs = mutedParticipants
            .map(p => p.local
                ? document.getElementById('localVideoTileViewContainer')
                : document.getElementById(`participant_${p.id}`))
            .filter(d => d);
        if (mutedDOMs.length) {
            allVideosContainers.append(...mutedDOMs);
        }
    },
